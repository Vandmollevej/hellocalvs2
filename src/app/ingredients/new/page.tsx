...
import { PRIVATE_INGREDIENT_PREFIX } from "@/lib/vault/handlers/private-ingredients";
...
function NewIngredientContent() {
  ...
  useEffect(() => {
    if (!useId) return;
    localApi(`/api/private-ingredients/${encodeURIComponent(useId)}`)
      .then(async (res) => {
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    try {
      let id = useId;
      if (!id) {
        const res = await localApi("/api/private-ingredients", {
          method: "POST",
