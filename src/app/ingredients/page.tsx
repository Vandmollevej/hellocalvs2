...
async function fetchIngredients(): Promise<PrivateIngredient[]> {
  const res = await localApi("/api/private-ingredients").catch(() => null);
...
export default function PrivateIngredientsPage() {
  ...
  async function saveName(id: string) {
    ...
    await localApi(`/api/private-ingredients/${encodeURIComponent(id)}`, {
      method: "PATCH",
  ...
  async function remove(ingredient: PrivateIngredient) {
    ...
    await localApi(`/api/private-ingredients/${encodeURIComponent(ingredient.id)}`, { method: "DELETE" }).catch(
      () => null
    );
    await load();
  }
