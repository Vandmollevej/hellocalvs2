...
function IntegrationerContent() {
  ...
  async function createToken(label: string) {
    ...
    try {
      const response = await fetch("/api/integrations/healthkit/tokens", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        // docs/PRIVACY.md: data fra appen forsegles til en ny, anonym
        // indbakke, som kun denne boks kan åbne.
        body: JSON.stringify({ label, inboxId: await createInbox() }),
      });
      if (response.ok) {
  ...
  async function connect(slug: string) {
    ...
    try {
      ...
      window.location.assign(`/api/integrations/${slug}/connect?inbox=${encodeURIComponent(inboxId)}`);
    } catch {
