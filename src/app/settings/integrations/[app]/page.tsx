  ...
  if (loading || !integration) {
    return (
      <HfScreen title={t("integrations.title")}>
        <p className="hf-type-small text-text-secondary p-4 text-center">{loading ? t("integrations.loading") : "—"}</p>
      </HfScreen>
    );
  }
      ...
      <div className="hf-page">
        ...
        <div className="flex items-start gap-3 rounded-[8px] bg-hf-tan p-4">
          ...
          <div className="min-w-0 flex-1">
            ...
            {integration.lastSyncedAt && (
              <p className="hf-type-micro text-text-secondary mt-1">
            ...
            {integration.lastError && <p className="hf-type-micro mt-1 text-hf-red-dark">{integration.lastError}</p>}
          </div>
        ...
        {integration.provider === "SAMSUNG_HEALTH" && (
          <div className="flex flex-col gap-3 rounded-[8px] bg-hf-tan p-4">
            <p className="hf-type-small text-text-secondary">{t("integrations.samsungViaHealthConnect")}</p>
            <Link
        ...
        {readTypes.length > 0 && (
          <>
            {!connected && <p className="hf-type-small text-text-secondary px-1">{t("integrations.chooseFirst")}</p>}
            <p className="hf-type-small hf-type-strong px-1 text-hf-black">{t("integrations.readTitle")}</p>
            {toggleRows("read", readTypes)}
            <p className="hf-type-small hf-type-strong px-1 text-hf-black">{t("integrations.writeTitle", { name })}</p>
            {writeTypes.length > 0 ? (
              toggleRows("write", writeTypes)
            ) : (
              <p className="hf-type-small text-text-secondary px-1">{t("integrations.writeNone", { name })}</p>
            )}
          </>
        )}

        {saveError && <p className="hf-type-small px-1 text-hf-red-dark">{t("integrations.saveError")}</p>}
        ...
        {isOAuth && connected && integration.needsReconnect.length > 0 && (
          <div className="flex flex-col gap-2 rounded-[8px] bg-hf-black p-4 text-hf-white">
            <p className="hf-type-small">{t("integrations.needsReconnect", { name })}</p>
            <button
              type="button"
              disabled={busy || !integration.configured}
              onClick={connect}
              className="hf-btn-primary w-full py-2.5 disabled:opacity-50"
            >
          ...
          (connected ? (
            <div className="flex gap-2">
              <button
                ...
                disabled={busy}
                onClick={sync}
                className="hf-btn-primary flex-1 py-2.5 disabled:opacity-50"
              >
              ...
              <button
                ...
                disabled={busy}
                onClick={disconnect}
                className="hf-type-small hf-type-strong flex-1 rounded-full bg-hf-tan py-2.5 text-hf-black disabled:opacity-50"
              >
        ...
        {integration.issuesDeviceTokens && (
          <div className="flex flex-col gap-2 rounded-[8px] bg-hf-tan p-4">
            <p className="hf-type-small text-text-secondary">{t("integrations.companionHowTo")}</p>

            {cardTokens.map((token) => (
              <div key={token.id} className="flex items-center justify-between gap-2 rounded-[8px] bg-hf-cream px-3 py-2">
                <p className="hf-type-micro text-text-secondary min-w-0 truncate">
                ...
                <button
                  ...
                  className="hf-type-small hf-type-strong shrink-0 text-hf-red-dark disabled:opacity-50"
                >
            ...
            {newToken ? (
              <div className="rounded-[8px] bg-hf-black p-3 text-hf-white">
                <p className="hf-type-small hf-type-strong">{t("integrations.saveTokenNotice")}</p>
                <p className="hf-type-small mt-1 break-all font-mono">{newToken}</p>
                <button type="button" onClick={() => setNewToken(null)} className="hf-btn-text mt-2">
              ...
              <button
                ...
                disabled={busy}
                onClick={createToken}
                className="hf-type-small hf-type-strong w-full rounded-full bg-hf-cream py-2.5 text-hf-black disabled:opacity-50"
              >
