...
// Admin "Uncertainties" (docs/DECISIONS.md 2026-09-24): fire faner, én række
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-wrap items-center gap-2 text-xs text-text-muted">
        ...
        <span className="ml-auto">Mål: mindst {Math.round(target * 100)} % sikkerhed (vejledende)</span>
      </div>
      ...
      {visible.length === 0 ? (
        ...
        <div className="flex flex-col gap-2">
          {visible.map((row) => (
            <div
              key={row.analysisId}
              className="flex items-center gap-3 rounded-lg border border-border-strong bg-surface-2 p-3"
            >
              ...
              <span
                className={`w-14 text-right text-sm font-semibold ${
                  row.uncertaintyPercent >= 50 ? "text-hf-red-dark" : "text-text-primary"
                }`}
              >
      ...
      {editing && (
        <Overlay onClose={() => setEditing(null)}>
          <EditLightbox
            ...
            onSaved={() => {
              setRows((current) => current.filter((r) => r.analysisId !== editing.analysisId));
              setEditing(null);
              router.refresh();
            }}
          />
...
function EditLightbox({ row, fields, onSaved }: { row: UncertaintyRow; fields: Field[]; onSaved: () => void }) {
  ...
  async function save() {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/admin/uncertainties/${row.analysisId}`, {
        method: "PATCH",
