    ...
    <div className="relative rounded-2xl bg-hf-tan p-4">
        ...
        {visibleSeries.map((s) => {
          ...
          return (
            <g key={s.key}>
              ...
              />
              {points.map((p, i) => (
                <circle key={i} cx={p.x} cy={p.y} r="3.2" fill={s.color} />
              ))}
            </g>
          );
        })}
      ...
      <div className="mt-2 flex items-center justify-between">
        <div className="flex flex-wrap items-center gap-3">
          {visibleSeries.map((s) => (
            <span key={s.key} className="flex items-center gap-1.5 text-xs text-hf-black opacity-70">
              <span
                aria-hidden="true"
        ...
        <button
          ...
          className="flex flex-col items-center gap-0.5 rounded-lg p-1 text-hf-black opacity-70 hover:opacity-100 focus-visible:outline-2 focus-visible:outline-hf-black"
        >
