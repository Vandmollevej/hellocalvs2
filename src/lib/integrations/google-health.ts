...
async function weights(accessToken: string, since: Date): Promise<InboxEnvelope[]> {
...
async function exercises(accessToken: string, since: Date): Promise<InboxEnvelope[]> {
...
async function dailySteps(accessToken: string, since: Date): Promise<InboxEnvelope[]> {
...
export const googleHealth: OAuthProviderAdapter = {
  ...
  async fetchItems(accessToken, since) {
    ...
    const ok = results.filter((r): r is PromiseFulfilledResult<InboxEnvelope[]> => r.status === "fulfilled");
