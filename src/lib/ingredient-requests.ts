...
// ingrediens ligger kun i boksen; serveren kender kun navnet og en anonym
// engangsindbakke. Admin varsles og kan oprette ingrediensen globalt — så
// sendes den globale ingrediens til indbakken, og enheden erstatter den
// private med den (src/lib/vault/handlers/private-ingredients.ts).
...
// Indbakke-typen enheden lytter efter (src/lib/vault/handlers/inbox.ts).
export const GLOBAL_INGREDIENT_INBOX_KIND = "globalIngredient";

export async function createIngredientRequest(name: string, replyInboxId: string | null) {
  const request = await prisma.ingredientRequest.create({ data: { name, replyInboxId } });
...
// giver besked til den anonyme indbakke.
export async function addIngredientRequestGlobally(requestId: string, name: string, adminUserId: string) {
  ...
  if (request.replyInboxId) {
    await deliverToInbox(request.replyInboxId, [
      {
        kind: GLOBAL_INGREDIENT_INBOX_KIND,
        payload: { requestId: request.id, productId: ingredient.id, name: ingredient.name },
      },
    ]).catch((error) => console.error("Ingredient request inbox delivery failed", error));
  }
  return ingredient;
}
