...
- Branch: `master`
- Latest committed checkpoint: `f342929` — secure server access workflow.
- The local branch is two commits ahead of `origin/master` before the current
  uncommitted deployment work.
...
1. Review and commit the production deployment checkpoint.
2. Push `master` and verify both GHCR image tags are built successfully.
3. Transfer the production Compose and environment template to the new
   `/volume1/docker/App/hellocal-v2` server directory.
4. Start the isolated stack, then verify migrations, health, product lookup,
   registration, persistence, backup, and rollback.
5. Test a temporary Cloudflare hostname against NAS port `3100` before changing
   any existing route.
