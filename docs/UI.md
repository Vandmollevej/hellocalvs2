...
- Øverste bjælke (på forsiden): venstre en cirkel med brugerens initialer — tryk åbner en kort menu: Profil, Indstillinger, Privatliv, Log ud. (Undersider: tilbagepil i venstre hjørne, profilcirkel i højre — se Layout-konsistens.)
...
- Ved siden af +-knappen: et lodret nøgletalspanel udformet som et drejehjul/urskive. Hvert tal står på én linje med sit ikon til HØJRE for tallet — ikke foran (brugerens gentagne krav, senest 2026-09-25; erstatter den tidligere regel om ikon foran tallet). Det midterste tal er altid størst og tydeligst (kalorier som standard) med et flamme-ikon og står helt vandret; tallene over og under bliver gradvist mindre og svagere, hver med sit eget ikon (fx en gåfigur for skridt, et æg-ikon for protein). Der er samme luft mellem alle rækker, og der vises op til 3 tal over og 3 under midten. Rækkerne hælder en anelse som en vifte: dem over midten med venstre ende opad, dem under med venstre ende nedad, lidt mere pr. række — kun få grader, ikke som en sol. Tallene må aldrig beskæres, heller ikke hvis de bliver lange. Lodret swipe, scroll, tryk på nabotallet eller tastaturets pile roterer panelet, så et nyt tal bliver midtstillet og forstørret.
...
- ~~Under det store centrale tal vises totalen (fx "/ 3.299 kcal") som en mindre linje nedenunder~~ — udgået 2026-09-25: brugeren vil kun have én linje pr. tal i drejehjulet, uden "/ mål"-linje.
...
- Den samme topbjælke og den samme bundnavigation (fire ikoner) vises konsekvent på alle skærme — også Tilføj, Kamera, Stemme og Søgning. Indholdet skifter, men rammen foroven og forneden er identisk overalt.
- Topbjælken har faste slots: tilbagepil i **venstre** hjørne, titel i midten, profilcirkel i **højre** hjørne. Tilbagepilen vises automatisk på alle routede sider undtagen de sider, der åbnes direkte fra bundnavigationen (standard: Tilføj, Madvarer, Kalender, Statistik); nested routes under dem har pil. Pilen er den fælles SVG-chevron (`HfChevron`), hvid, uden tekst eller cirkel. Der bruges aldrig et ✕/kryds til at lukke eller navigere tilbage nogetsteds i appen, jf. `docs/DECISIONS.md` (2026-09-22 "Global tilbage-navigation").
...
- ~~Arbejdstider i kalenderen~~ — udgået 2026-09-22: der er ikke længere et separat guide-spørgsmål eller en toggle for at registrere arbejdstider i kalenderen (`workHoursInCalendarEnabled`). Registrering af arbejdstid hører under "Skiftende arbejdstider" (`shiftWorkEnabled`). Se `docs/DECISIONS.md` (2026-09-22).
