...
## 2026-09-25: Automatisk produktgenkendelse i kameraet (fanen "Produkt")

- Fanen "Produkt" (`/camera?mode=detect`; gamle `mode=hellofresh` peger samme sted) har ingen udløserknap. Siden sender selv et billede til AI, når kameraet holdes stille over noget nyt (lille gråtonemål i `src/lib/camera-frame.ts`; ved rystende hånd sendes alligevel efter 4 s). Brugerens ønske: "Man skal ikke trykke på kameraet".
- `POST /api/ai/detect-camera-objects` (kræver login) bruger `callStructuredVision` med `OPENAI_CAMERA_VISION_MODEL` (standard: `OPENAI_PRODUCT_VISION_MODEL`). AI afgør pr. objekt: rund tallerken/skål → ring (ellipse) om tallerkenen; alt andet → streg langs objektets kontur. Koordinater er brøkdele af det kvadratiske udsnit, brugeren ser.
- Hver fødevare matches mod egne godkendte produkter (`textSimilarity` ≥ 0,6); uden match bruges AI's estimat, markeret "AI-estimat". Pakkede produkter får en typisk portion, ikke hele pakken.
- Første svar med mad fryser billedet med omridsene, kameraet krymper og listen ruller frem nedenfor: tryk på en række med produktmatch åbner `/add/[id]`; "Registrér alle" gemmer via `/api/registrations`; "Scan igen" genstarter.
- HelloFresh-retter indgår kun i matchet, når kameraet er åbnet fra Opret ret (`for=ret`) — lukker det åbne punkt i 2026-09-24 "HelloFresh kun i Opret ret". `/api/ai/recognize-hellofresh` bruges ikke længere af kameraet.
- Sort søger: kamerastart er nu serialiseret. Ved faneskift stoppede @zxing's forsinkede `stop()` den nye strøm på det fælles `<video>`-element. Derudover genstarter en vagt kameraet (højst 2 gange), hvis videoen ingen billeder har 2,5 s efter start.
